...
.extra {padding: 1rem 2rem 2rem 2rem;}
...
function expand(element) {
    ...
    while(item && item.nodeType != Node.ELEMENT_NODE)
...
TRACE_STYLES = """\
.extra {font-family: Consolas, monaco, monospace; font-size: 0.4em;}
...
.extra .error
{
    ...
    color: gold; background-color: brown;
...
.items
{
    margin-top: 0.2rem; border-top: 1px dashed gray; padding: 0.2rem 0rem 0rem 1.8rem;
}

.items > div
{
    display: flex;
    border-radius: 2px; padding: 0rem 0.25rem 0rem 0.25rem;
...
.items > div > div:not(:only-of-type):first-of-type
{
    flex: 0 0 35rem;
}

.items > div > div:not(:only-of-type):not(:last-of-type):after
{
    content: "\\00a0\
...
.items > div > div:not(:only-of-type):not(:first-of-type):before
{
    content: "\\00a0";
}

.items > div > div:not(:only-of-type):not(:first-of-type)
{
    overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
    flex: 1 1 1rem;
}
...
.expand
{
    ...
    border: 1px solid rgba(0, 0, 0, 0.075); border-radius: 2px;
    padding: 0.25rem;
...
.expand:hover
{
    border-color: rgba(0, 0, 0, 0); background-color: rgba(0, 0, 0, 0.05);
}
