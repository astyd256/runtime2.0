...
def build_wholexml_data(layout, widgets):
    ...
    for key,item in iter_items:
        ...
        for event,actions in copy.deepcopy(events).iteritems():
            ...
            events_part[preffix + event] = actions


    js_code = LIB_JLAYOUT_ALL_JS + INIT_JLAYOUT_JS % (json.dumps(layout), )


    result_vdomxml = RESULT_VDOMXML_TEMPLATE.format(**{
        'widgets_part'  : '\n'.join(widgets_part),
        'js_code'       : js_code,
    })
